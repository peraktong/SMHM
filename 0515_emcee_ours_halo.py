...
class MCMC():
    ...
    def calculate_mass(self):
        ...
        # For Halo, we need to input Mh


        cal_fq = np.vectorize(self.f_q)

        f_q_array_temp = np.array(cal_fq(Mh=self.Mh))
        # print(z_array)
...
def lnlike(theta, x, y):
    ...
    # Theta is a tuple
    f0,mhc,sigmah,alphah = theta

    kwargs["f0"] = f0
    kwargs["zc"] = mhc
    kwargs["sigmaz"] = sigmah
    kwargs["alphaz"] = alphah
...
def lnprior(theta):
    ...
    if 0 < f0 < 3 and 8 < mhc < 14 and 0 < sigmah < 5 and 2 < alphah < 9:
        return 0.0
    return -np.inf
...
result = op.minimize(nll, [kwargs["f0"],kwargs["mhc"],kwargs["sigmah"],kwargs["alphah"]], args=(x, y))
print("finish doing scipy opt")

print("result x")
print(result["x"])
f0,mhc,sigmah,alphah = result["x"]
...
output = open('EMCEE_halo_M13_scipy.pkl', 'wb')
pickle.dump(result["x"], output)
output.close()

output = open('kwargs_halo.pkl', 'wb')
...
output = open('EMCEE_halo_M13.pkl', 'wb')
