...
class MCMC():
    ...
    def f_con_Jason(self, a, Mh, Ms):
        ...
        return f0 * ((Mh / mht) ** A1) * ((Ms / mst) ** A2) * ((1. + z) ** A3)* (exp(-(at - a) ** 2 / As))
...
model = MCMC(kwargs=kwargs)
model.fitting_separately(index=all_index[2])

counter = 0

# From older EMCEE chain
...
def lnlike(theta, x, y):
    ...
    print(chi, f0, A1, A2, A3, A4, A5,zs)
...
def lnprior(theta):
    ...
    # Let A4<0!!
    if 0 < f0 < 9 and -5 < A1 < 5 and -5 < A2 < 5 and -15 < A3 < 15 and -1 < A4 < 0.01 and -3 < A5 < 3and 0 < zs < 5:
        return 0.0
    return -np.inf
...
result = op.minimize(nll, [kwargs["f0"], kwargs["A1"], kwargs["A2"], kwargs["A3"], kwargs["A4"], kwargs["A5"], kwargs["zs"]],
                     args=(x, y))
...
pos = [
    [kwargs["f0"], kwargs["A1"], kwargs["A2"], kwargs["A3"], kwargs["A4"], kwargs["A5"], kwargs["zs"]] + 1e-3 * np.random.randn(
