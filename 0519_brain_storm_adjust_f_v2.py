...
class MCMC():
    ...
    def calculate_median_us(self):
        ...
        if (scatter_0 > 0.2):
            # &(np.sum((m_Behroozi - m_us_chi) ** 2)<1)&(self.kwargs["MCMC"]!=1)
...
def lnprior(theta):
    ...
    if 0 < f0 < 9 and -5 < A1 < 5 and -5 < A2 < 5 and -15 < A3 < 15 :
        return 0.0
    return -np.inf
...
output = open('EMCEE_new_fcon_M12_scipy_4.pkl', 'wb')
pickle.dump(result["x"], output)
output.close()

output = open('kwargs_new_fcon_4.pkl', 'wb')
...
output = open('EMCEE_new_fcon_M12_4.pkl', 'wb')
