...
def lnprior(theta):
    ...
    # Let A4<0!!
    if 0 < f0 < 1 and -1 < A1 < 1 and -5 < A2 < -0.1 and -1.5 < A3 < 1.5 and -1 < A4 < 2 and -1.5 < A5 < 1.5 and 0.001 < A6 < 1:
        return 0.0
    return -np.inf
...
result = op.minimize(nll,
                     [kwargs["f0"], kwargs["A1"], kwargs["A2"], kwargs["A3"], kwargs["A4"], kwargs["A5"], kwargs["A6"]],
                     args=(x, y))
...
save_path = "Fast_Simultaneously_best_fit_v2.pkl"
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(x, y), threads=-1)
