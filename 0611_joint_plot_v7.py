...
class MCMC():
    ...
    def f_con_Jason(self, a, Mh, Ms):
        ...
        at = A4 * log10(self.M_h_median) + A5
        ...
        return f0 * ((Mh / mht) ** A1) * ((Ms / mst) ** A2) * ((1. + z) ** A3)* (math.exp(-(at - a) ** 2 / As))
    ...
    def Plot_M_stellar(self):
        ...
        for halo in range(1, n_halo):
            ...
            self.M_h_median = np.nanmedian(M_h)
        ...
        plt.plot([], [], "ko", label="f0=%.2f A1=%.2f A2 = %.2f A3 = %.2f A4=%.2f A5=%.2f As=%.2f" % (
            kwargs["f0"] * math.exp(slope), kwargs["A1"], kwargs["A2"], kwargs["A3"], kwargs["A4"], kwargs["A5"], kwargs["zs"]))
        ...
        save_path = plot_path + "Fitting_separately_v7" + str(self.index).replace(".","") + ".png"
    ...
    def Plot_20_Halo_f_con(self):
        ...
        save_path = plot_path + "f_con_Fitting_separately_v7" + str(self.index).replace(".","") + ".png"
        fig.savefig(save_path, dpi=200)
...
class diagnostic():
    ...
    def walker_plot(self):
        ...
        save_path = "/Users/caojunzhi/Downloads/upload_0530_Jeremy/" + "Walker_plot_v7_" + ".png"
        fig.savefig(save_path, dpi=200)
    ...
    def corner_plot(self, fusion):
        ...
        fig = corner.corner(samples, labels=name,
                            truths=[f0[0], A1[0], A2[0], A3[0], A4[0], A5[0], zs[0]])

        # fig = corner.corner(samples, labels=name)


        fig.savefig("/Users/caojunzhi/Downloads/upload_0530_Jeremy/" + "triangle_v7.png")
...
kwargs = {"MCMC": None, "A1": None, "A2": None, "A3": None, "f0": None, "A4": None, "A5": None, "zs": None, "mht": None,
...
save_path = "Separately_best_fit_M11.0_v7.pkl"
...
save_path = "Separately_EMCEEM11.0_v7.pkl".replace("M11.0", str(model.index))
...
f0, A1, A2, A3, A4, A5, As = map(lambda v: (v[1], v[2] - v[1], v[1] - v[0]),
                                 zip(*np.percentile(result[50:,:], [16, 50, 84],
                                                    axis=0)))

print(f0[0], A1[0], A2[0], A3[0], A4[0], A5[0], As[0])
